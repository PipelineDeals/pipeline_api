...
describe Pipeline::Deal do
  it_should_behave_like "a paginated collection"

  let(:docs_obj) { deal }
  let(:cals_obj) { deal }
  let(:notes_obj) { deal }
  let(:people_obj) { VCR.use_cassette(:deal_with_primary_contact) { Pipeline::Deal.find 2 } }
  ...
  let(:deal) { VCR.use_cassette(:get_a_deal) { Pipeline::Deal.find 1 } }
  let(:deal_with_contact) { VCR.use_cassette(:deal_with_primary_contact) { Pipeline::Deal.find 2 } }
  let(:deal_with_company) { VCR.use_cassette(:deal_with_company) { Pipeline::Deal.find 81 } }

  it "creates a deal" do
    VCR.use_cassette(:deal_create) do
      new_deal = Pipeline::Deal.create(:name => 'a new deal')
  ...
  describe "filters" do
    it "on stage" do
      VCR.use_cassette(:deals_filtered_by_stage) do
        deals = Pipeline::Deal.where(conditions: { deal_stage: [2,4] })
    ...
    it "on numeric custom field" do
      VCR.use_cassette(:deals_filtered_by_custom_field) do
        deals = Pipeline::Deal.where(conditions: {custom_label_11: { from: 3, to: 7 }})
    ...
    it "on picklist custom field" do
      VCR.use_cassette(:deals_filtered_by_custom_field) do
        deals = Pipeline::Deal.where(conditions: {custom_label_12: [19,20]})
        expect(deals.size).to eq 1
  ...
  describe "associations" do
    it "has a deal stage" do
      expect(deal.deal_stage).to be_an_instance_of Pipeline::DealStage
      expect(deal.deal_stage.name).to eq "Qualified Lead"
    end

    it "has custom fields" do
      expect(deal.custom_fields).to be_an_instance_of Pipeline::Deal::CustomFields
    ...
    it "can update custom fields" do
      VCR.use_cassette(:update_deal) do
        ...
        deal.save
        custom_label = Pipeline::Deal.find(1).custom_fields.custom_label_25
        expect(custom_label).to eq 1
      end
    end

    it "has a source" do
      expect(deal.source).to be_an_instance_of Pipeline::LeadSource
      expect(deal.source.name).to eq "Cold Call"
    end

    it "can update source" do
      VCR.use_cassette(:update_lead_source) do
        source = Pipeline::LeadSource.last
    ...
    it "has a user" do
      expect(deal.user).to be_an_instance_of(Pipeline::User)
    end

    it "has a primary contact" do
      expect(deal_with_contact.primary_contact).to be_an_instance_of Pipeline::Person
    end

    it "has a company" do
      expect(deal_with_company.company).to be_an_instance_of Pipeline::Company
    end
